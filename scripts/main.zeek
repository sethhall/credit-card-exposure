...
export {
	...
	option use_cc_separators = F;

	## Separators expected to be seen between groups of digits in a credit card.
	option cc_separators = /\.([0-9]*\.){2}/ | /\-([0-9]*\-){2}/ |
	    /[:blank:]([0-9]*[:blank:]){2}/;

	global trimmed_redacted_data: string;
}
...
function check_cards(c: connection, data: string): bool
	...
	for ( ccp in ccps )
		...
		if ( ( ! use_cc_separators || cc_separators in ccp ) && luhn_check(ccp) )
			...
			local log: Info = [ $ts=network_time(), $uid=c$uid, $id=c$id,
			    ...
			    $data=( redact_log ? trimmed_redacted_data :
			    sub_bytes(data, begin, byte_count) ) ];
	...
	if ( found_cnt > 0 )
		{
		NOTICE([ $note=CreditCardExposure::Found, $conn=c, $msg=fmt(
		    "Found at least %d credit card number%s", found_cnt,
		    found_cnt > 1 ? "s" : ""), $sub=trimmed_redacted_data,
...
event CreditCardExposure::stream_data(f: fa_file, data: string)
	...
	if ( c$start_time > network_time() - 20 secs )
		check_cards(c, data);
	}
